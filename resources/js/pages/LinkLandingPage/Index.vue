...
const headers = [
    ...
    { text: "Aksi", value: "id", sortable: false, width: 160 },
];
...
const loadFromServer = async () => {
    ...
    const { data } = await axios.get(route('link-landing-page.index'), {
        params: {
...
function onCreate() {
    router.get(route('link-landing-page.create'))
}

function onEdit(id: number) {
    router.visit(route('link-landing-page.edit', id))
}
...
function onDelete(id: number) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete(route('link-landing-page.destroy', id))
                .then(response => {
                    Swal.fire({
                        title: 'Terhapus!',
...
const toggleStatus = async (id: number) => {
    try {
        await axios.put(route('link-landing-page.toggle', id))
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4 space-y-4">
            <div class="container mx-auto flex items-center justify-between mb-4">
                <Heading title="Manajemen Link Landing Page"
                    description="Daftar link landing page yang telah terdaftar dalam sistem" class="!mb-0" />
            ...
            <EasyDataTable v-model:server-options="serverOptions" :server-items-length="serverItemsLength"
                ...
                <!-- Header -->
                <template #header-index>No</template>
                ...
                <template #item-link="{ link }">
                    <a :href="link" class="text-blue-600 underline" target="_blank">
                        {{ link }}
                    </a>
                </template>
